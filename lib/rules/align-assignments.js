        ...
        context.report({
          ...
          fix:     (fixer) =>  {
            const fixings = group.map(function(node) {
              ...
              const assignment = tokens.find(token => token.value === '=');

              const line          = sourceCode.getText(node);
              const lineIsAligned = line.charAt(maxPos) === '=';
              if (lineIsAligned || !assignment)
                return fixer.replaceText(node, line);
              else {
                // source line may include space, we need to accomodate for that.
                const spacePrefix    = tokens[0].loc.start.column;
                const startDelimiter = assignment.loc.start.column - spacePrefix;
                const endDelimiter   = assignment.loc.end.column - spacePrefix;
