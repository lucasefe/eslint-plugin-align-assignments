  ...
  create(context) {
    ...
    return {
      ...
      ExpressionStatement(node) {
        ...
        const hasAssignmentExpression = node.expression && node.expression.type === 'AssignmentExpression';
    ...
    function getPrefix(node) {
      const preNode = isAssignmentExpression(node) ?
        node.left :
        node.declarations[0].id;

      const prefix = preNode.loc.end.column - preNode.loc.start.column;
      return prefix;
    }
